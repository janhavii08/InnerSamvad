...
class _MoodTrackerScreenState extends State<MoodTrackerScreen>
  ...
  // ✅ UPDATED FUNCTION: Saves mood locally + online (Firebase)
  void _saveMood() async {
    final user = FirebaseAuth.instance.currentUser;

    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please log in to save your mood online.')),
      );
      return;
    }
    ...
    try {
      ...
      print("🟢 Uploading to Firebase: $moodData");
      ...
      print("✅ Mood saved in Firebase: ${ref.id}");
    } catch (e) {
      print('❌ Error saving to Firebase: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Firebase save failed: $e')),
      );
    }
    ...
    // Debug: confirm how many entries exist online
    try {
      final snapshot = await FirebaseFirestore.instance
          ...
          .collection('entries')
          .get();
      print("🟩 Total moods in Firebase: ${snapshot.docs.length}");
    } catch (e) {
      print("⚠️ Could not fetch Firebase mood count: $e");
    }
  }
  ...
  List<FlSpot> _buildWeeklySpots(Map<String, double> weekly) {
    ...
    for (var i = 0; i < keys.length; i++) {
      spots.add(FlSpot(i.toDouble(), weekly[keys[i]] ?? 0.0));
    }
    return spots;
  }
            ...
            children: [
              ...
              Expanded(
                child: TabBarView(
                  controller: _tabController,
                  children: [
                    _buildDailyTab(grouped, dailyCounts),
                    _buildWeeklyTab(weekly, weeklySpots),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildDailyTab(Map<DateTime, List<MoodEntry>> grouped, Map<String, int> dailyCounts) {
                ...
                Column(
                  children: dailyCounts.entries.map((e) {
                    final percent = (e.value /
                            dailyCounts.values.fold<int>(0, (p, n) => p + n) *
                            100)
                        .round();
                    return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Builder(builder: (context) {
                  final entries = _entriesForDay(_selectedDay ?? DateTime.now());
                  if (entries.isEmpty) {
  ...
  Widget _buildWeeklyTab(Map<String, double> weekly, List<FlSpot> spots) {
                    ...
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            final label = _moodValue.entries
                                .firstWhere(
                                    (e) => e.value == value,
